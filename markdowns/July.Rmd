...
```{r length bias, echo = FALSE, message = FALSE, eval = FALSE}
library(tidyverse)
...
I fit some GAMs on the male and female un-weighted means for Total Length bin sizes of 8 using `gam()` in the `mgcv` package. For females, the gam fit with wiggliness and looks fine. I used "REML" method. But for males, the outlier at the lower bin length is making the fit just a linear regression (edf = 1) with really poor fit. When I remove the outlier, the fit looks similar to the females and theres a better fit ($r^2$).

...
![Figure 41 from 2017 lingcod stock assessment](July_images/lingcod_south_growthcurve_stockassess.png){height="25%"}
...
![Figure from Bonnie et al.](July_images/percent.abundance.w.png){height="25%"}
...
![All Ports](July_images/dietcomp_AllPorts.jpeg){height="25%"}  
![California Ports only](July_images/dietcomp_CAonly.jpeg){height="25%"}
...
![male GAM w/ outlier](July_images/male_gam_w_outlier.jpeg){height="25%"}
...
![female age-length key](July_images/female_growth_curve.jpeg){height="25%"} ![male age-length key](July_images/male_growth_curves.jpeg){height="25%"}

and it looks like they match pretty well, so I am going to go ahead and use the age-length key to determine length intervals for each age.

First, I went back to my age-length keys to get the upper and lower limit of lengths (binned by 5cm) and fit a gam to the upper interval across ages and a gam to the lower interval across ages for both males (plus group is aggregated starting at age 11+) and females (plus group is aggregated starting at age 12+) separately. Then, I predicted the upper and lower intervals for each age using the gam, took the difference between the upper and lower value and divided by 2 to get the variation going one way from the mean (assuming variation is equal on both sides) $interval = (UI - LI)/2$.

Next, I will use this vector of length variation for each age to subset the diet data according to length at age (from pop model) +/- the variation calculated from diet data and use *P. Moriarty et al.*'s method to determine diet fraction for each age. Again, imo, it is fine to use the age-length information from the diet data interchangeably with the pop model age-length estimation since the growth curves are fairly similar (see above).

I created a function that will first subset the data to the sex of interest and the length range for a given age, then run the model, and paste the $c_i$ output into a empty vector for diet fractions.

Interestingly, according to the mixture model, males were consuming more rockfish than females.

![diet fractions](July_images/diet_fractions.jpeg){height="25%"}


**For next week:**  
- Try to fit a gam using `gamlss` package that uses a zero- and one-inflated beta distribution to the diet data (family = BEINF).  
- Prepare a brief summary of the progress I've made on the lingcod-rockfish project to update Marissa.  
